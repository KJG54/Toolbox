...
class Settings(BaseSettings):
    ...
    whisper_model: str = Field(
        # LOCAL DEFAULT: 'auto' picks by free RAM, so the model silently changes
        # run to run. Pinned for predictable CPU transcription time.
        default="small", description="faster-whisper model size, or 'auto' by RAM/VRAM."
    )
    whisper_device: str = Field(
        # LOCAL DEFAULT: CUDA faster-whisper needs cuDNN/cuBLAS DLLs that are not
        # installed here, and the GPU is reserved for the Ollama vision model.
        default="cpu", description="faster-whisper device: 'auto', 'cpu', or 'cuda'."
    )
    ...
    ollama_num_ctx: int = Field(
        # LOCAL DEFAULT: 2048 upstream is too small — a 4-image critic batch is
        # ~2177 tokens and Ollama returns HTTP 400. 8192 fits with the model
        # still 100% on GPU (measured 5.1/6.1 GiB on the RTX 2060).
        default=8192,
    ...
    # --- vision tiers ------------------------------------------------------
    # LOCAL DEFAULTS: keyless local Ollama on both tiers, so every agent works
    # from any working directory without a .env or inherited env vars.
    # Override with WATCHSKILL_VISION_* to switch to a cloud provider.
    vision_cheap_provider: str = Field(
        default="ollama", description="Provider for bulk scene descriptions."
    )
    vision_cheap_model: str = Field(default="qwen2.5vl:3b")
    vision_strong_provider: str = Field(
        default="ollama", description="Provider for final answers and critiques."
    )
    vision_strong_model: str = Field(default="qwen2.5vl:3b")
    ...
    vision_batch_size: int = Field(
        # LOCAL DEFAULT: 1 — qwen2.5vl:3b is a small local model; batching
        # multiple frames per call degrades output and inflates context.
        default=1,
    ...
    critic_frame_cap: int = Field(
        # LOCAL DEFAULT: 4 — keeps the critic batch inside the 8192 context.
        default=4,
