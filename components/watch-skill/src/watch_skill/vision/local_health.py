...
from watch_skill.proc import background
...
def restart_ollama_detached() -> bool:
    ...
    if sys.platform == "win32":
        # background() = CREATE_NEW_PROCESS_GROUP + console suppression, and
        # deliberately NOT DETACHED_PROCESS. `ollama serve` forks a runner per
        # model load; under DETACHED_PROCESS it has no console to hand down, so
        # each runner opened its own visible terminal mid-pipeline. The server
        # still outlives us — Windows does not reap children on parent exit.
        kwargs = background(**kwargs)
    else:
